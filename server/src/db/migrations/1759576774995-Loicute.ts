...
export class Loicute1759576774995 implements MigrationInterface {
    name = 'Loicute1759576774995'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`doctor\` (\`id\` varchar(36) NOT NULL, \`full_name\` varchar(255) NOT NULL, \`gender\` varchar(255) NULL, \`biography\` varchar(255) NULL, \`specialization\` varchar(255) NOT NULL, \`experience_years\` int NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, \`isActive\` tinyint NOT NULL DEFAULT 1, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`role_staff\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`description\` varchar(255) NULL, UNIQUE INDEX \`IDX_10116bfbec7d8595f53540cb4b\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`staff\` (\`id\` varchar(36) NOT NULL, \`avatar\` varchar(255) NULL, \`full_name\` varchar(255) NOT NULL, \`phone\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL, \`isActive\` tinyint NOT NULL DEFAULT 1, \`roleId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`staff\` ADD CONSTRAINT \`FK_10288fdeaacf288bbd4d6469fef\` FOREIGN KEY (\`roleId\`) REFERENCES \`role_staff\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`appointment_history\` ADD CONSTRAINT \`FK_4403cfa08cd458a6374ee13f615\` FOREIGN KEY (\`changedByStaffId\`) REFERENCES \`staff\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`staff\` DROP FOREIGN KEY \`FK_10288fdeaacf288bbd4d6469fef\``);
        ...
        await queryRunner.query(`DROP INDEX \`IDX_10116bfbec7d8595f53540cb4b\` ON \`role_staff\``);
        await queryRunner.query(`DROP TABLE \`role_staff\``);
